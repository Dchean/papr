...
/// The feed URL for a feed id — used by the sync layer to propagate feed
/// mutations (unsubscribe / move / URL change) to the server.
...
/// The folder a feed sits in (by name, not id) — used by sync to propagate a
/// local folder move server-side.
...
/// The folder name for a folder id — used by sync to propagate folder
/// create / rename / delete operations to the server.
...
/// Feed URLs the sync layer should mirror onto the server, paired with the
/// name of the folder each sits in (if any) so a newly pushed subscription can
/// be filed into the same folder server-side. Excludes `newsletter` sources.
...
/// Local article id for a remote (GReader) item id. Miniflux's ids are a
/// stable `tag:google.com,2005:reader/item/<hex>` form, so a pull can map a
/// remote item straight onto its local row without relying on URL equality
/// (which breaks when the server's canonical href differs from the feed
/// document's own link).
pub fn article_id_by_remote_id(conn: &Connection, remote_id: &str) -> AppResult<Option<i64>> {
    Ok(conn
        .query_row(
            "SELECT id FROM articles WHERE remote_id = ?1 LIMIT 1",
            params![remote_id],
            |r| r.get(0),
        )
        .optional()?)
...
/// Local article id for a feed-scoped GUID — the pair `upsert_article` keys
/// on, so a just-imported remote item can be found again.
